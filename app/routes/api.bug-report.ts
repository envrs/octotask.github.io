...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.octotaskVersion) {
      body += `- octotask.diy: ${data.environmentInfo.octotaskVersion}\n`;
    }
  ...
  body += '---\n*Submitted via octotask.diy bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/octotask.diy';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'octotask.diy-bug-reporter',
    });
